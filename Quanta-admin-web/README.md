...
以下是源码中已经定义并由页面调用的接口。当前默认连接真实后端；只有登记在仓库根目录 `docs/Quanta管理端Web接口Mock清单.md` 中的接口会使用局部 Mock。
...
管理端已默认连接真实后端，本机开发地址为 `http://127.0.0.1:8080`。截至 2026-09-14，登录、用户权限、控制台、成员、招新、活动报名、工位、图书、塔服和学习资料的读取接口均已完成真实后端冒烟。

当前仅保留两项明确登记的局部兜底：后端动态菜单组件名与当前 Web 组件映射不兼容，以及联调库的成员届次数据为空。具体原因、临时行为和取消条件见仓库根目录的 `docs/Quanta管理端Web接口Mock清单.md`。写接口和文件下载接口不会因失败自动回退 Mock。

生产构建按 `/admin/` 子路径发布；Nginx 需将 `/admin/` 回退到 Web 的 `index.html`，API 继续按部署环境配置转发。

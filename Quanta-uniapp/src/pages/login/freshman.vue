<template>
	<view class="freshman-page">
		<view class="top-bar">
			...
			<text class="page-title">新生登录</text>
...
//定义学号和密码的响应式变量并且通过v-model绑定到输入框上
const freshman_id = ref('')
const freshman_password = ref('')

const loading = ref(false)

//验证学号和密码是否为空
const validateFreshmanLogin = () =>{
	//学号非空校验 只输入空格也算空
  if(!freshman_id.value.trim()) {
    uni.showToast({  //提示用户输入学号
      title: '请输入学号',
      icon: 'none'
    })
    return false
  ...
  //密码非空校验 只输入空格也算空
  if(!freshman_password.value.trim()) {
    uni.showToast({  //提示用户输入密码
      title: '请输入密码',
      icon: 'none'
    })
    return false
  }
	freshman_id.value = freshman_id.value.trim()
	freshman_password.value = freshman_password.value.trim()
  return true
}
...
const handleLogin = async() => {
	...
	try {
		const res = await loginApi({
			username: freshman_id.value,
			password: freshman_password.value,
			loginType: '0',
		})

		await completeLogin(res, 'freshman', freshman_id.value)
		uni.showToast({
			title: res.msg || '登录成功',
			icon: 'success',
		})
		setTimeout(() => {
			uni.reLaunch({ url: '/pages/freshman/home' })
		}, 500)
	} catch (error) {
		uni.showToast({

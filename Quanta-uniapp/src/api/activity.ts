...
import type { MockActivity, LocalActivitySignup } from '../utils/mockActivity'
...
export const mapSignup = (row: SignupDto): LocalActivitySignup => ({
  activityId: Number(row.activityId),
  status: row.status,
...
export const getMyActivitySignup = async (activityId: number) => {
  const response = await request<TableResponse<SignupDto>>({
...
export const signupActivity = async (activityId: number, remark = '') => {
  await request({ url: '/system/signup', method: 'POST', data: { activityId, remark } })
